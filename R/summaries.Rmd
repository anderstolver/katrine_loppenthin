...
tmp_data <- select(data_full, ID_NUM, time, Randomisering, psqi_total)
tmp_data <- spread(tmp_data, key = time, value = psqi_total)
tmp_data <- mutate(tmp_data, change = follow_up - baseline)
tmp_data <- group_by(tmp_data, Randomisering) %>%
  summarise_at(.vars = "change", .funs = my_funs ) %>%
  filter(!is.na(Randomisering)) %>%
  mutate(time = c("baseline", "follow_up"))
tmp_data
